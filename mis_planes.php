...
<!-- Internal Styles for Custom Colors -->
<style>
    /* Emerald Accent Color */
    .text-emerald-accent { color: #0df280; }
    .bg-emerald-accent { background-color: #0df280; }
    .border-emerald-accent { border-color: #0df280; }
    .hover\:bg-emerald-accent:hover { background-color: #0df280; }
    .group:hover .group-hover\:text-emerald-accent { color: #0df280; }
</style>
...
<div class="flex flex-col md:flex-row justify-between items-start md:items-end gap-4 mb-8">
    <div class="flex flex-col gap-1">
        ...
        <p class="text-slate-400">Manage and review workouts assigned to your athletes</p>
    </div>
...
<div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
    <div class="bg-slate-800 border border-slate-700 p-4 rounded-xl flex items-center gap-4 group transition-colors hover:border-emerald-accent/50">
        <div class="p-3 bg-emerald-accent/10 rounded-lg text-emerald-accent">
            <i data-lucide="layers" class="w-6 h-6"></i>
        </div>
        <div>
            <p class="text-slate-400 text-xs font-medium uppercase">Total Plans</p>
            <p class="text-xl font-bold text-white group-hover:text-emerald-accent transition-colors"><?php echo $plansSummary['total_plans'] ?? 0; ?></p>
        </div>
    </div>
    <div class="bg-slate-800 border border-slate-700 p-4 rounded-xl flex items-center gap-4 group transition-colors hover:border-amber-500/50">
        <div class="p-3 bg-amber-500/10 rounded-lg text-amber-400">
            <i data-lucide="clock" class="w-6 h-6"></i>
        </div>
        <div>
            <p class="text-slate-400 text-xs font-medium uppercase">Pending</p>
            <p class="text-xl font-bold text-white group-hover:text-amber-400 transition-colors"><?php echo $plansSummary['pending_count'] ?? 0; ?></p>
        </div>
    </div>
    <div class="bg-slate-800 border border-slate-700 p-4 rounded-xl flex items-center gap-4 group transition-colors hover:border-blue-500/50">
        ...
        <div>
            ...
            <p class="text-xl font-bold text-white group-hover:text-blue-400 transition-colors"><?php echo $plansSummary['completed_no_feedback'] ?? 0; ?></p>
        </div>
    </div>
    <div class="bg-slate-800 border border-slate-700 p-4 rounded-xl flex items-center gap-4 group transition-colors hover:border-purple-500/50">
        <div class="p-3 bg-purple-500/10 rounded-lg text-purple-400">
            <i data-lucide="message-circle" class="w-6 h-6"></i>
        </div>
        <div>
            <p class="text-slate-400 text-xs font-medium uppercase">Feedback</p>
            <p class="text-xl font-bold text-white group-hover:text-purple-400 transition-colors"><?php echo $plansSummary['with_feedback_count'] ?? 0; ?></p>
        </div>
    </div>
</div>
...
<?php if (empty($allWorkouts)): ?>
    <div class="bg-slate-800 border border-slate-700 rounded-xl p-12 text-center">
        ...
        <p class="text-slate-400 text-sm mb-6">Try adjusting your filters or create a new plan.</p>
        <a href="generar_plan.php"
            class="inline-flex items-center gap-2 px-6 py-2 bg-emerald-accent text-[#0f172a] text-sm font-bold rounded-lg hover:bg-[#0bc568] transition-colors shadow-lg shadow-emerald-accent/20">
        ...
        <?php foreach ($allWorkouts as $workout): ?>
            ...
            if ($workout['status'] === 'pending') {
                ...
                if ($workout['coach_feedback']) {
                    ...
                    $statusColor = 'text-emerald-accent';
            ...
            <!-- Card -->
            <div
                class="bg-slate-800 border <?php echo $borderColor; ?> rounded-xl p-5 flex flex-col gap-4 relative overflow-hidden group hover:shadow-xl transition-all duration-300">
                <!-- Hover Glow Effect -->
                <div
                    class="absolute top-0 right-0 w-32 h-32 <?php echo $accentClass; ?>/5 rounded-bl-full -mr-8 -mt-8 transition-all group-hover:<?php echo $accentClass; ?>/15">
                </div>
                ...
                <div class="flex items-start justify-between relative z-10">
                    ...
                    <span
                        class="px-2 py-1 rounded-md text-xs font-bold bg-[#0f172a] border border-slate-700 <?php echo $statusColor; ?> flex items-center gap-1">
                ...
                <div class="relative z-10">
                    <div class="flex items-center justify-between mb-2">
                        <span class="text-xs font-bold text-white bg-slate-700/50 px-2 py-0.5 rounded uppercase tracking-wider border border-slate-700">
                            <?php echo htmlspecialchars($workout['type']); ?>
                        </span>
                    </div>
                    <p class="text-slate-400 text-sm line-clamp-2 h-10">
                        <?php echo htmlspecialchars($workout['description'] ?? 'No description provided.'); ?>
                    </p>
                </div>

                <!-- Stats / Actions -->
                <div class="mt-auto pt-4 border-t border-slate-700 flex justify-between items-center relative z-10">
                    <div class="flex flex-col">
                        <span class="text-[10px] text-slate-500 uppercase font-bold">Planned Dist</span>
                    ...
                    <button onclick='openDetailModal(<?php echo json_encode($workout); ?>)'
                        class="text-xs bg-[#0f172a] text-white hover:bg-emerald-accent hover:text-[#0f172a] border border-slate-700 hover:border-emerald-accent px-4 py-2 rounded-lg transition-all font-bold flex items-center gap-2">
                        View Details
...
<div id="detailModal" class="fixed inset-0 bg-black/80 backdrop-blur-sm hidden items-center justify-center z-50">
    <div
        class="bg-slate-800 border border-slate-700 rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto m-4 shadow-2xl">
        <div class="p-6 border-b border-slate-700 flex justify-between items-center">
            <div>
                <h3 class="text-xl font-bold text-white" id="modalTitle">Workout Details</h3>
                <p class="text-slate-400 text-sm mt-1" id="modalSubtitle">Complete training information</p>
            </div>
    ...
    function openDetailModal(workout) {
        ...
        let html = `
            <div class="grid grid-cols-2 gap-4 mb-6">
                <div>
                    <label class="text-xs uppercase font-bold text-slate-500">Athlete</label>
            ...
            <div class="bg-[#0f172a] rounded-xl p-4 border border-slate-700 mb-6">
                 <div class="flex items-center justify-between mb-2">
                    <label class="text-xs uppercase font-bold text-slate-500">Workout Description</label>
                    <span class="text-xs font-bold text-emerald-accent bg-emerald-accent/10 px-2 py-0.5 rounded">${workout.type}</span>
                </div>
                <p class="text-slate-300 leading-relaxed">${workout.description || 'No description provided.'}</p>
            </div>
        `;

        if (workout.status === 'completed') {
            html += `
                <div class="bg-[#0f172a] rounded-xl p-4 border border-slate-700 space-y-3">
                    <h4 class="text-sm font-bold text-white flex items-center gap-2">
                        <i data-lucide="activity" class="w-4 h-4 text-emerald-accent"></i> Athlete Results
                    </h4>
                    <div class="grid grid-cols-3 gap-4 text-sm">
                        <div class="bg-slate-800 p-2 rounded border border-slate-700">
                            <span class="text-slate-400 text-xs block">Distance</span>
                        ...
                        <div class="bg-slate-800 p-2 rounded border border-slate-700">
                            <span class="text-slate-400 text-xs block">RPE</span>
        ...
        if (workout.feedback) {
            html += `
                <div class="bg-slate-800 rounded-xl p-4 border border-l-4 border-slate-700 border-l-emerald-accent mt-4">
                    <h4 class="text-xs uppercase font-bold text-emerald-accent mb-2 flex items-center gap-2">
        ...
        if (workout.coach_feedback) {
            html += `
                <div class="bg-slate-800 rounded-xl p-4 border border-l-4 border-slate-700 border-l-purple-500 mt-4">
