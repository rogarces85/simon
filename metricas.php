...
<div class="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
    <div>
        ...
        <p class="text-slate-400 text-lg">Track your team's fitness progression and compliance.</p>
    </div>
    <div class="flex items-center gap-3">
        ...
        <div class="relative">
            <form method="GET" id="filterForm">
                <select name="athlete_id" onchange="this.form.submit()"
                    class="pl-4 pr-10 py-2.5 rounded-lg bg-slate-800 border border-slate-700 text-white text-sm font-medium hover:bg-slate-700 transition-colors appearance-none cursor-pointer outline-none focus:ring-2 focus:ring-emerald-accent/50">
        ...
        <button
            class="flex items-center gap-2 bg-emerald-accent text-[#0f172a] px-4 py-2.5 rounded-lg text-sm font-bold hover:bg-[#0bc568] transition-colors shadow-lg shadow-emerald-accent/10">
...
<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
    ...
    <div
        class="bg-slate-800 rounded-xl p-6 border border-slate-700 hover:border-emerald-accent/30 transition-colors group">
        <div class="flex justify-between items-start mb-4">
            <div class="p-2 bg-emerald-accent/10 rounded-lg text-emerald-accent">
                <i data-lucide="users" class="w-6 h-6"></i>
            </div>
            <span
                class="flex items-center text-emerald-accent text-sm font-medium bg-emerald-accent/10 px-2 py-1 rounded">
                <i data-lucide="trending-up" class="w-3 h-3 mr-1"></i> +1
            </span>
        </div>
        <p class="text-slate-400 text-sm font-medium mb-1">Active Athletes</p>
        <p class="text-white text-3xl font-bold tracking-tight group-hover:text-emerald-accent transition-colors">
            <?php echo $athleteId === 'all' ? count($athletes) : 1; ?>
        </p>
    </div>

    <!-- Card 2: Completed Workouts -->
    <div class="bg-slate-800 rounded-xl p-6 border border-slate-700 hover:border-blue-500/30 transition-colors group">
        <div class="flex justify-between items-start mb-4">
            <div class="p-2 bg-blue-500/10 rounded-lg text-blue-400">
                <i data-lucide="check-circle" class="w-6 h-6"></i>
            </div>
            <span class="flex items-center text-blue-400 text-sm font-medium bg-blue-500/10 px-2 py-1 rounded">
                <i data-lucide="trending-up" class="w-3 h-3 mr-1"></i> +12%
            </span>
        </div>
        <p class="text-slate-400 text-sm font-medium mb-1">Completed (All Time)</p>
        <p class="text-white text-3xl font-bold tracking-tight group-hover:text-blue-400 transition-colors">
            <?php echo $completedCount; ?>
        </p>
    </div>

    <!-- Card 3: Pending This Week -->
    <div class="bg-slate-800 rounded-xl p-6 border border-slate-700 hover:border-amber-500/30 transition-colors group">
        <div class="flex justify-between items-start mb-4">
            <div class="p-2 bg-amber-500/10 rounded-lg text-amber-500">
                <i data-lucide="clock" class="w-6 h-6"></i>
            </div>
            <span class="flex items-center text-amber-500 text-sm font-medium bg-amber-500/10 px-2 py-1 rounded">
                <i data-lucide="alert-circle" class="w-3 h-3 mr-1"></i> Due
            </span>
        </div>
        <p class="text-slate-400 text-sm font-medium mb-1">Pending This Week</p>
        <p class="text-white text-3xl font-bold tracking-tight group-hover:text-amber-500 transition-colors">
            <?php echo $pendingThisWeek; ?>
        </p>
    </div>

    <!-- Card 4: Compliance -->
    <div class="bg-slate-800 rounded-xl p-6 border border-slate-700 hover:border-purple-500/30 transition-colors group">
        <div class="flex justify-between items-start mb-4">
            <div class="p-2 bg-purple-500/10 rounded-lg text-purple-400">
                <i data-lucide="activity" class="w-6 h-6"></i>
            </div>
            <span class="flex items-center text-purple-400 text-sm font-medium bg-purple-500/10 px-2 py-1 rounded">
                <i data-lucide="bar-chart-2" class="w-3 h-3 mr-1"></i> Rate
            </span>
        </div>
        <p class="text-slate-400 text-sm font-medium mb-1">Compliance Rate</p>
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
    ...
    <!-- Large Chart Area -->
    <div class="lg:col-span-2 bg-slate-800 rounded-xl border border-slate-700 p-6 lg:p-8 flex flex-col min-h-[420px]">
        <?php if ($athleteId !== 'all' && isset($progressionData)): ?>
            <div class="flex justify-between items-start mb-6">
                <div>
                    <h3 class="text-white text-lg font-bold">Weekly Volume</h3>
                    <p class="text-slate-400 text-sm">Distance (km) vs Pace (min/km) over last 8 weeks</p>
                </div>
                <div class="flex items-center gap-4">
                    <div class="flex items-center gap-2">
                        <span class="w-3 h-3 rounded-full bg-emerald-accent"></span>
        ...
        <?php else: ?>
            <div class="flex justify-between items-start mb-6">
                <div>
                    ...
                    <p class="text-slate-400 text-sm">Compliance per Athlete</p>
                </div>
            </div>
            <?php if (empty($athletesMetrics)): ?>
                <div class="flex flex-col items-center justify-center flex-1 text-slate-500">
                    ...
                    <table class="w-full text-left">
                        ...
                        <tbody class="text-sm">
                            <?php foreach ($athletesMetrics as $m): ?>
                                <tr class="border-b border-slate-700 last:border-0 group hover:bg-slate-700/50 transition-colors">
                                    <td class="py-3 pl-2 font-bold text-white"><?php echo htmlspecialchars($m['athlete_name']); ?>
                                    </td>
                                    <td class="py-3 text-slate-400"><?php echo number_format($m['total_distance'], 1); ?> km</td>
                                    <td class="py-3">
                                        <div class="flex items-center gap-2">
                                            <div class="w-24 h-1.5 bg-slate-700 rounded-full overflow-hidden">
                                                <div class="h-full bg-emerald-accent"
                                                    style="width: <?php echo $m['compliance_rate']; ?>%">
                                                </div>
                                            </div>
                                    ...
                                    <td class="py-3 text-right">
                                        <a href="metricas.php?athlete_id=<?php echo $m['athlete_id']; ?>"
                                            class="text-emerald-accent hover:text-white transition-colors text-xs font-bold uppercase">View</a>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
                        </tbody>
    ...
    <div class="flex flex-col gap-4">
            ...
            <div class="flex flex-col gap-3">
                ...
                <?php foreach ($recentWorkouts as $workout): ?>
                    ...
                    $iconData = match ($workout['type']) {
                        ...
                        'Intervalos' => ['icon' => 'timer', 'bg' => 'bg-amber-500/20', 'text' => 'text-amber-400'],
                        'Recuperación' => ['icon' => 'heart', 'bg' => 'bg-emerald-500/20', 'text' => 'text-emerald-400'],
                        default => ['icon' => 'activity', 'bg' => 'bg-slate-700', 'text' => 'text-white']
                    };
                    ?>
                    <div
                        class="flex items-center justify-between p-3 rounded-lg hover:bg-slate-700/50 transition-colors cursor-pointer group border border-transparent hover:border-slate-700">
                        <div class="flex items-center gap-3">
                            <div
                            ...
                            <div>
                                ...
                                <p class="text-slate-400 text-xs"><?php echo htmlspecialchars($workout['athlete_name']); ?>
                                    • <?php echo (new DateTime($workout['date']))->format('M j'); ?></p>
                            </div>
                        </div>
                        <div class="text-right">
                            <?php if ($workout['status'] === 'completed'): ?>
                                <p class="text-white text-sm font-bold group-hover:text-emerald-accent transition-colors">
                                    <?php echo $workout['actual_distance']; ?> km
                                </p>
                                <p class="text-slate-400 text-xs"><?php echo $workout['rpe']; ?> RPE</p>
                            <?php else: ?>
                                <span
                                    class="text-xs text-amber-500 font-bold bg-amber-500/10 px-2 py-0.5 rounded">Pending</span>
                            <?php endif; ?>
                        </div>
                    </div>
                <?php endforeach; ?>
        ...
        <div
            class="bg-gradient-to-br from-slate-800 to-slate-900 rounded-xl border border-slate-700 p-6 relative overflow-hidden">
            <div class="absolute top-0 right-0 p-4 opacity-10">
                <i data-lucide="activity" class="w-24 h-24 text-white"></i>
            </div>
            <div class="relative z-10">
                <p class="text-slate-400 text-sm font-medium mb-1">Team Avg RPE</p>
                <div class="flex items-baseline gap-2">
                    <p class="text-white text-4xl font-black">7.2</p>
                    <p class="text-emerald-accent text-sm font-bold flex items-center">
                        <i data-lucide="arrow-up" class="w-3 h-3"></i> 0.4
                    </p>
                </div>
                <div class="mt-4 w-full bg-slate-700 h-1.5 rounded-full overflow-hidden">
                    <div class="bg-emerald-accent h-full rounded-full" style="width: 72%"></div>
                </div>
                <p class="text-slate-400 text-xs mt-2">Moderate / Hard Intensity</p>
            </div>
        </div>
    </div>
</div>
...
// NEW: Recent Activity for Sidebar
$recentStmt = $db->prepare("SELECT w.*, u.name as athlete_name FROM workouts w $whereClause ORDER BY w.date DESC LIMIT
5");
$recentStmt->execute($params);
$recentWorkouts = $recentStmt->fetchAll(PDO::FETCH_ASSOC);
